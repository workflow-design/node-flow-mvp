...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { transactionId: txId, cost } = await checkAndDeductCredits(
      user.id,
      "fal-ai/veo3.1/fast/keyframe"
    );
    transactionId = txId;

    console.log("Veo 3.1 Fast Keyframe API: calling fal-ai/veo3.1/fast/first-last-frame-to-video");
    ...
    console.error("Veo 3.1 Fast Keyframe API error:", error);
