...
import type { Veo31FastKeyframeNodeData } from "@/types/nodes";
...
import { veo31FastKeyframeGenerators } from "@/lib/workflow/executors";
...
export function Veo31FastKeyframeNode({ id, data }: NodeProps<Veo31FastKeyframeNodeData>) {
  ...
  const updateNodeData = useCallback(
    (updates: Partial<Veo31FastKeyframeNodeData>) => {
      setNodes((nodes) =>
        nodes.map((node) =>
  ...
  const generateVideo = useCallback(
    async (prompt: string, firstFrameUrl?: string, lastFrameUrl?: string): Promise<{ url: string; type: "image" | "video" }> => {
      const url = await veo31FastKeyframeGenerators.frontend(prompt, firstFrameUrl, lastFrameUrl);
  ...
  const handleRun = useCallback(async () => {
    ...
    const firstFrameUrl = inputs.firstFrame;
    const lastFrameUrl = inputs.lastFrame;
    ...
    if (promptInput.items && promptInput.items.length > 0) {
      ...
      await executeBatch({
        ...
        galleryNodeId,
        executeModel: (prompt) => generateVideo(prompt, firstFrameUrl, lastFrameUrl),
      ...
      try {
        const result = await generateVideo(promptInput.value, firstFrameUrl, lastFrameUrl);
