...
import type { Veo31FastNodeData } from "@/types/nodes";
...
import { veo31FastGenerators } from "@/lib/workflow/executors";

const INPUT_HANDLES = [{ id: "prompt", label: "prompt", required: true, type: "text" as const }];

export function Veo31FastNode({ id, data }: NodeProps<Veo31FastNodeData>) {
  ...
  const updateNodeData = useCallback(
    (updates: Partial<Veo31FastNodeData>) => {
      setNodes((nodes) =>
        nodes.map((node) =>
  ...
  const generateVideo = useCallback(
    async (prompt: string): Promise<{ url: string; type: "image" | "video" }> => {
      const url = await veo31FastGenerators.frontend(prompt);
  ...
  const handleRun = useCallback(async () => {
    ...
    if (promptInput.items && promptInput.items.length > 0) {
      ...
      try {
        const result = await generateVideo(promptInput.value);
  ...
  }, [getInputsWithMeta, findConnectedOutputGallery, updateNodeData, executeBatch, generateVideo]);
