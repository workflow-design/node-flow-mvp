...
import type { Veo31I2vNodeData } from "@/types/nodes";
...
import { veo31I2vGenerators } from "@/lib/workflow/executors";
...
export function Veo31I2vNode({ id, data }: NodeProps<Veo31I2vNodeData>) {
  ...
  const updateNodeData = useCallback(
    (updates: Partial<Veo31I2vNodeData>) => {
      setNodes((nodes) =>
        nodes.map((node) =>
  ...
  const generateVideo = useCallback(
    async (prompt: string, imageUrl?: string): Promise<{ url: string; type: "image" | "video" }> => {
      const url = await veo31I2vGenerators.frontend(prompt, imageUrl);
  ...
  const handleRun = useCallback(async () => {
    ...
    if (!imageUrl) {
      updateNodeData({
        status: "error",
        error: "No image connected. Connect an Image node to the image input.",
      });
      return;
    }
