...
import type { Veo31KeyframeNodeData } from "@/types/nodes";
...
export function Veo31KeyframeNode({ id, data }: NodeProps<Veo31KeyframeNodeData>) {
  ...
  const updateNodeData = useCallback(
    (updates: Partial<Veo31KeyframeNodeData>) => {
      setNodes((nodes) =>
        nodes.map((node) =>
  ...
  const generateVideo = useCallback(
    async (prompt: string, firstFrameUrl?: string, lastFrameUrl?: string): Promise<{ url: string; type: "image" | "video" }> => {
      const url = await veo31KeyframeGenerators.frontend(prompt, firstFrameUrl, lastFrameUrl);
