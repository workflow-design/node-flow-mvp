...
/**
 * Creates a FluxDev executor with the given image generator.
 * This allows the same batch/error handling logic to be shared
 * between frontend (API routes) and backend (direct Fal calls).
 */
export function createFluxDevExecutor(generateImage: ImageGenerator): NodeExecutor {
  return {
    async execute(
      _node: AppNode,
 ...
 */
export const fluxDevGenerators = {
  /**
 ...
 */
export const fluxDevExecutor = createFluxDevExecutor(fluxDevGenerators.backend);
