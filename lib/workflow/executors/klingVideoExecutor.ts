...
/**
 * Creates a Kling Video executor with the given video generator.
 */
export function createKlingVideoExecutor(generateVideo: VideoGenerator): NodeExecutor {
  return {
    async execute(
      _node: AppNode,
 ...
 */
export const klingVideoGenerators = {
  /**
  ...
  frontend: async (prompt: string): Promise<string> => {
    const response = await fetch("/api/fal/kling-video", {
      method: "POST",
 ...
 */
export const klingVideoExecutor = createKlingVideoExecutor(klingVideoGenerators.backend);
