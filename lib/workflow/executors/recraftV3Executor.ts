...
/**
 * Creates a Recraft V3 executor with the given image generator.
 */
export function createRecraftV3Executor(generateImage: ImageGenerator): NodeExecutor {
  return {
    async execute(
      _node: AppNode,
 ...
 */
export const recraftV3Generators = {
  /**
  ...
  frontend: async (prompt: string): Promise<string> => {
    const response = await fetch("/api/fal/recraft-v3", {
      method: "POST",
 ...
 */
export const recraftV3Executor = createRecraftV3Executor(recraftV3Generators.backend);
