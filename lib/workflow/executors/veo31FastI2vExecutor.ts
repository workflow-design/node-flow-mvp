...
export function createVeo31FastI2vExecutor(generateVideo: VideoGenerator): NodeExecutor {
  return {
    async execute(
      _node: AppNode,
      ...
      if (!imageInput?.value) {
        return {
          output: { value: "", type: "video" },
          status: "failed",
          error: "No image input connected - required for image-to-video",
        };
      }
...
export const veo31FastI2vGenerators = {
  ...
  frontend: async (prompt: string, imageUrl?: string): Promise<string> => {
    const response = await fetch("/api/fal/veo31-fast-i2v", {
      method: "POST",
...
export const veo31FastI2vExecutor = createVeo31FastI2vExecutor(veo31FastI2vGenerators.backend);
