...
export function createVeo31FastKeyframeExecutor(generateVideo: VideoGenerator): NodeExecutor {
  return {
    async execute(
      _node: AppNode,
...
export const veo31FastKeyframeGenerators = {
  ...
  frontend: async (prompt: string, firstFrameUrl?: string, lastFrameUrl?: string): Promise<string> => {
    const response = await fetch("/api/fal/veo31-fast-keyframe", {
      method: "POST",
...
export const veo31FastKeyframeExecutor = createVeo31FastKeyframeExecutor(veo31FastKeyframeGenerators.backend);
