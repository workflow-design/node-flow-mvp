...
export function createVeo31I2vExecutor(generateVideo: VideoGenerator): NodeExecutor {
  return {
    async execute(
      _node: AppNode,
...
export const veo31I2vGenerators = {
  ...
  frontend: async (prompt: string, imageUrl?: string): Promise<string> => {
    const response = await fetch("/api/fal/veo31-i2v", {
      method: "POST",
...
export const veo31I2vExecutor = createVeo31I2vExecutor(veo31I2vGenerators.backend);
